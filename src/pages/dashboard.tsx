...
export default function Dashboard() {
  ...
  const handleLogoutClick = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/v1/test/sign-out/${encodeURIComponent(user.email)}`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const propertyResponse = await fetch("http://localhost:3001/v1/test/property");
        const propertyData = await propertyResponse.json();

        const userResponse = await fetch("http://localhost:3001/v1/test/User-dash");
        const userData = await userResponse.json();

        const pendingResponse = await fetch("http://localhost:3001/v1/test/property-fetching/PENDING");
        const pendingData = await pendingResponse.json();

        const approvedResponse = await fetch("http://localhost:3001/v1/test/property-fetching/APPROVED");
